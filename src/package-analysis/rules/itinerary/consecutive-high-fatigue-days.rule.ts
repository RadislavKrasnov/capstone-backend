...
export class ConsecutiveHighFatigueDaysRule implements RecommendationRule {
  ...
  evaluate(context: AnalysisContext): RecommendationDraft[] {
    ...
    const dailyResults = [...(context.dailyFatigueResults ?? [])].sort((a, b) => a.dayNumber - b.dayNumber);

    for (let index = 1; index < dailyResults.length; index += 1) {
      const previous = dailyResults[index - 1];
      const current = dailyResults[index];

      if (previous.fatigueScore >= 70 && current.fatigueScore >= 70) {
        recommendations.push({
          ruleCode: this.code,
          category: this.category,
          severity: RecommendationSeverity.HIGH,
          title: `Days ${previous.dayNumber}-${current.dayNumber} are consecutively intensive`,
          explanation: 'Two high-fatigue days in a row increase the risk of poor traveler experience.',
          suggestedAction: 'Insert a lighter day, free-time block, or shorter transfer between these days.',
